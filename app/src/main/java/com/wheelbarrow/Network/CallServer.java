...
import com.wheelbarrow.Components.ComponentMacros.DatagramType;
...
public class CallServer extends CallObj implements AutoCloseable {
    ...
    private void supplyVideo(byte[] videoData) {
        ...
        if (videoData.length > minMTU) {
            for(int i = 0; i < videoData.length; i += minMTU) {
                int end = Math.min(videoData.length, i + minMTU);
    ...
    private void handleReceive() {
        ...
        while (running) {
            ...
            } else if (request.getLength() <= minMTU) {
